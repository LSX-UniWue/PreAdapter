...
def load():
    ...
    if 'roberta_adapter' in args.backbone:
        args.apply_roberta_output = True
        args.apply_roberta_attention_output = True
    if 'bart_adapter' in args.backbone:
        args.apply_bart_output = True
        args.apply_bart_attention_output = True
    ...
    if args.baseline in ['ctr_kg', 'ctr_kg_qa_dragon']:
        args.apply_bert_output = True
        args.apply_roberta_output = True
        args.apply_bart_output = True
        args.apply_bert_attention_output = True
        ...
        if args.build_adapter_mask:
            args.use_imp = False
            args.build_adapter_capsule_mask = False
        else:
            args.build_adapter_capsule_mask = True
            args.use_imp = True
        if args.apply_two_layer_shared:
            args.apply_one_layer_shared = False
        else:
            args.apply_one_layer_shared = True
        # args.use_imp = True
        args.transfer_route = True
        args.share_conv = True
        if args.baseline in ['ctr_kg']:
            args.larger_as_share = True
        args.adapter_size = True
