...
class RobertaAdapter(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.fc1 = torch.nn.Linear(config.roberta_hidden_size, config.roberta_adapter_size)
        self.fc2 = torch.nn.Linear(config.roberta_adapter_size, config.roberta_hidden_size)
        if config.use_gelu:
            self.activation = torch.nn.GELU()
        else:
            self.activation = torch.nn.ReLU()
        print('RobertaAdapter')
...
class RobertaAdapterMask(RobertaAdapter):
    def __init__(self, config):
        ...
        self.efc1 = torch.nn.Embedding(config.ntasks, config.roberta_adapter_size)
        self.efc2 = torch.nn.Embedding(config.ntasks, config.roberta_hidden_size)
        self.gate = torch.nn.Sigmoid()
        self.config = config
        self.smax = config.smax

        print('RobertaAdapterMask')
    ...
    def mask(self, t, s=1):
        if torch.cuda.is_available():
            ...
            efc1 = self.efc1(torch.LongTensor([t]).cpu())
            efc2 = self.efc2(torch.LongTensor([t]).cpu())

        gfc1 = self.gate(s * efc1)
        gfc2 = self.gate(s * efc2)


        if s == self.smax:  # you want to use it for evaluation
            gfc1 = (gfc1 > 0.5).float()
            gfc2 = (gfc2 > 0.5).float()
...
class RobertaAdapterCapsuleMaskImp(RobertaAdapterMask):
    '''
    ...
    def __init__(self, config):
        ...
        self.config = config
        print('RobertaAdapterCapsuleMaskImp')
...
class CapsuleLayerImp(nn.Module):  # it has its own number of capsule for output
        ...
        if layer_type == 'semantic':
            if config.apply_one_layer_shared:
                ...
                self.fc1 = nn.ModuleList(
                    [torch.nn.Linear(config.roberta_hidden_size, config.semantic_cap_size) for _ in range(config.ntasks)])

            elif config.apply_two_layer_shared:
                print('apply_two_layer_shared ')
                self.fc1 = nn.ModuleList(
                    [torch.nn.Linear(config.roberta_hidden_size, config.mid_size) for _ in range(config.ntasks)])
                self.fc2 = nn.ModuleList(
                    [torch.nn.Linear(config.mid_size, config.semantic_cap_size) for _ in range(config.ntasks)])
            print('CapsuleLayer')
            self.gelu = torch.nn.GELU()
            ...
            self.route_weights = \
                nn.Parameter(torch.randn(config.num_semantic_cap, self.num_routes, config.semantic_cap_size,
                                         config.semantic_cap_size))

            if config.exp in ['2layer_whole', '2layer_aspect_transfer']:
                self.elarger = torch.nn.Embedding(config.ntasks, config.roberta_hidden_size)
                self.larger = torch.nn.Linear(3 * config.ntasks,
                                              config.roberta_hidden_size)  # each task has its own larger way
            else:
                self.elarger = torch.nn.Embedding(config.ntasks, config.roberta_hidden_size)
                self.larger = torch.nn.Linear(config.semantic_cap_size * config.num_semantic_cap,
                                              config.roberta_hidden_size)  # each task has its own larger way

            if config.no_tsv_mask:
                if torch.cuda.is_available():  # cpu run if else
                    self.tsv = torch.ones(config.ntasks, config.ntasks).data.cuda()  # for backward
                else:
                    self.tsv = torch.ones(config.ntasks, config.ntasks).data.cpu()  # for backward
            else:
                if torch.cuda.is_available():  # cpu run if else
                    self.tsv = torch.tril(torch.ones(config.ntasks, config.ntasks)).data.cuda()  # for backward
                else:
                    self.tsv = torch.tril(torch.ones(config.ntasks, config.ntasks)).data.cpu()  # for backward
            ...
            if config.larger_as_list:
                self.larger = nn.ModuleList([
                    torch.nn.Linear(
                        ...
                        config.roberta_hidden_size) for _ in range(config.ntasks)])  # each task has its own larger way
                self.elarger = torch.nn.Embedding(config.ntasks, config.roberta_hidden_size)  # for capusle only
    ...
    def forward(self, t, x, s, layer_type=None):
        ...
        if layer_type == 'semantic':
            ...
            for pre_t in range(self.config.ntasks):
                ...
                cur_v = cur_v.contiguous().view(batch_size, self.config.max_seq_length,
                                                self.config.num_semantic_cap * self.config.semantic_cap_size)
                ...
                feature = feature.contiguous().view(batch_size, self.config.max_seq_length,
                                                    self.config.num_semantic_cap * self.config.semantic_cap_size)
                # z = [F.tanh(conv(feature.transpose(1, 2))) for conv in self.convs1]  # [(N,Co,L), ...]*len(Ks)
                y = [F.relu(conv(feature.transpose(1, 2)) + self.fc_cur(cur_v).unsqueeze(2)) for conv in
                     self.convs2]  # mix information
            ...
            sim_attn = torch.cat(sim_attn, 2)

            vote_outputs = (self.tsv[t].data.view(1, 1, -1, 1, 1) *
                            sim_attn.repeat(self.config.max_seq_length, 1, 1).view(self.config.num_semantic_cap, -1,
                                                                                   self.config.ntasks, 1,
                                                                                   self.config.semantic_cap_size) *
                            decision_maker.repeat(self.config.max_seq_length, 1).view(1, -1, self.config.ntasks, 1,
                                                                                      1) * priors).sum(dim=2,
                                                                                                       keepdim=True)  # route
            ...
            if self.config.exp in ['2layer_whole']:  # task,transfer,representation
                ...
                for pre_t in range(self.config.ntasks):
                    if pre_t != t:
                        ...
                        z = [F.tanh(conv(feature.transpose(1, 2))) for conv in self.convs1]  # [(N,Co,L), ...]*len(Ks)
                        y = [F.relu(conv(feature.transpose(1, 2)) + self.fc_aspect(aspect_v).unsqueeze(2)) for conv in
                             self.convs2]
                        z = [i * j for i, j in zip(z, y)]
                        z = [F.max_pool1d(i, i.size(2)).squeeze(2) for i in z]  # [(N,Co), ...]*len(Ks)
                        z = torch.cat(z, 1)
    ...
    def mask(self, t, s):
        if torch.cuda.is_available():
            ...
            glarger = self.gate(s * self.elarger(torch.LongTensor([t]).cpu()))

        if s == self.smax:  # you want to use it for evluation
            glarger = (glarger > 0.5).float()
...
class RobertaAdapterCapsuleMask(RobertaAdapterMask):
    def __init__(self, config):
        super().__init__(config)
        ...
        # self.fc1=torch.nn.Linear(semantic_cap_size,adapter_size)
        self.config = config
        print('RobertaAdapterCapsuleMask')
...
class CapsuleLayer(nn.Module):  # it has its own number of capsule for output
    def __init__(self, config, layer_type):
        ...
        if layer_type == 'tsv':
            ...
            # self.in_channel = 100
            self.elarger = torch.nn.Embedding(config.ntasks, config.roberta_hidden_size)
            self.larger = torch.nn.Linear(config.semantic_cap_size,
                                          config.roberta_hidden_size)  # each task has its own larger way
            self.gate = torch.nn.Sigmoid()
            self.softmax = torch.nn.Softmax()
            self.num_iterations = 3
            ...
            if config.apply_one_layer_shared:
                ...
                print('apply_two_layer_shared ')
                self.fc1 = nn.ModuleList([torch.nn.Linear(config.roberta_hidden_size, 100) for _ in range(config.ntasks)])
...
class RobertaAdapterUcl(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.relu = torch.nn.ReLU()
        self.fc1 = BayesianLinear(config.roberta_hidden_size, config.roberta_adapter_size, ratio=config.ratio)
        self.fc2 = BayesianLinear(config.roberta_adapter_size, config.roberta_hidden_size, ratio=config.ratio)
        print('RobertaAdapterUcl')
...
# OWM MLP ==========
class RobertaAdapterOwm(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.relu = torch.nn.ReLU()
        self.fc4 = torch.nn.Linear(config.roberta_hidden_size * config.max_seq_length, config.roberta_adapter_size)
        self.fc1 = torch.nn.Linear(config.roberta_adapter_size, config.roberta_adapter_size)
        self.fc2 = torch.nn.Linear(config.roberta_adapter_size, config.roberta_adapter_size)
        self.fc3 = torch.nn.Linear(config.roberta_adapter_size, config.max_seq_length * config.roberta_hidden_size)
        self.dropout = torch.nn.Dropout(0.5)
        self.config = config
        print('RobertaAdapterOwm')
    ...
    def forward(self, x, sample=False):
        ...
        h = h.view(x.size(0), self.config.max_seq_length, self.config.roberta_hidden_size)
        x = x.view(x.size(0), self.config.max_seq_length, self.config.roberta_hidden_size)

        return {'outputs': x + h, 'x_list': x_list, 'h_list': h_list}



class RobertaAdapterCapsule(RobertaAdapter):
    def __init__(self, config):
        super().__init__(config)
        self.capsule_net = CapsNet(config)
        print('RobertaAdapterCapsule')
...
class RobertaAdapterCapsuleImp(RobertaAdapter):
    def __init__(self, config):
        super().__init__(config)
        self.capsule_net = CapsNetImp(config)
        self.gelu = torch.nn.GELU()
        print('RobertaAdapterCapsuleImp')
