...
import sys
from transformers.models.roberta.modeling_roberta import RobertaModel,RobertaOutput,RobertaSelfOutput, RobertaEncoder, RobertaLayer, RobertaAttention, RobertaSelfAttention
sys.path.append("./networks/base/")
from roberta_adapters import RobertaAdapterMask, RobertaAdapter, RobertaAdapterUcl, RobertaAdapterOwm
...
class MyRobertaSelfOutput(RobertaSelfOutput):
    def __init__(self, config,args):
        ...
        if args.use_imp:
            from networks.base.roberta_adapters import RobertaAdapterCapsuleMaskImp as RobertaAdapterCapsuleMask
            from networks.base.roberta_adapters import RobertaAdapterCapsuleImp as RobertaAdapterCapsule
        else:
            from networks.base.roberta_adapters import RobertaAdapterCapsuleMask
            from networks.base.roberta_adapters import RobertaAdapterCapsule


        if args.apply_roberta_attention_output:
            print('apply to attention')
            if args.build_adapter:
                self.adapter = RobertaAdapter(args)
            if args.build_adapter_ucl:
                self.adapter_ucl = RobertaAdapterUcl(args)
            if args.build_adapter_owm:
                self.adapter_owm = RobertaAdapterOwm(args)
            elif args.build_adapter_mask:
                self.adapter_mask = RobertaAdapterMask(args)
            elif args.build_adapter_capsule_mask:
                self.adapter_capsule_mask = RobertaAdapterCapsuleMask(args)
            elif args.build_adapter_capsule:
                self.adapter_capsule = RobertaAdapterCapsule(args)
...
class MyRobertaLayer(RobertaLayer):
    def __init__(self, config,args):
        super().__init__(config)
        self.attention = MyRobertaAttention(config,args)
        if self.add_cross_attention:
            assert self.is_decoder, f"{self} should be used as a decoder model if cross attention is added"
            self.crossattention = MyRobertaAttention(config,args)
    ...
    def forward(
        ...
        if self.args.apply_roberta_output or self.args.apply_roberta_attention_output:
            if self.args.build_adapter_capsule_mask: return {'outputs':outputs}
            elif self.args.build_adapter_owm: return {'outputs':outputs,'x_list':x_list,'h_list':h_list}
            else: return outputs
        else: return outputs
