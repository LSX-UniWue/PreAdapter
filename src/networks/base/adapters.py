        ...
        if layer_type == 'semantic':
            if config.apply_one_layer_shared:
                ...
                self.fc1 = nn.ModuleList(
                    [torch.nn.Linear(config.bert_hidden_size, config.semantic_cap_size) for _ in range(config.ntasks)])

            elif config.apply_two_layer_shared:
                print('apply_two_layer_shared ')
                self.fc1 = nn.ModuleList(
                    [torch.nn.Linear(config.bert_hidden_size, config.mid_size) for _ in range(config.ntasks)])
            ...
            if config.exp in ['2layer_whole', '2layer_aspect_transfer']:
                ...
                self.larger = torch.nn.Linear(3 * config.ntasks,
                                              config.bert_hidden_size)  # each task has its own larger way
            else:
                self.elarger = torch.nn.Embedding(config.ntasks, config.bert_hidden_size)
