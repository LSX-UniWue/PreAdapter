...
roberta_backbone = ['roberta','roberta_adapter','roberta_frozen']
w2v_backbone = ['w2v','w2v_as']

language_dataset = ['asc','dsc','ssc','nli','newsgroup',  'asc_2', 'asc_5', 'nlg', 'fb','fbl',
                    'qa_dragon']
...
if args.task == 'asc_2': #aspect sentiment classication
    if args.backbone=='w2v':
        from dataloaders.asc_2 import w2v as dataloader
    elif args.backbone=='w2v_as':
        from dataloaders.asc_2 import w2v_as as dataloader
    elif args.backbone in bert_backbone: #all others
        from dataloaders.asc_2 import bert as dataloader

if args.task == 'asc_5': #aspect sentiment classication
    if args.backbone=='w2v':
        from dataloaders.asc_5 import w2v as dataloader
    elif args.backbone=='w2v_as':
        from dataloaders.asc_5 import w2v_as as dataloader
    elif args.backbone in bert_backbone: #all others
        from dataloaders.asc_5 import bert as dataloader
...
elif args.task == 'qa_dragon':
    if args.backbone=='w2v':
        from dataloaders.qa_dragon import w2v as dataloader
    elif args.backbone in roberta_backbone:
        from dataloaders.qa_dragon import roberta as dataloader
...
if args.task in language_dataset:
    ...
    if args.backbone == 'bert':
        ...
        if args.baseline=='one_qa_dragon':
            from approaches.classification import bert_one_qa_dragon as approach
            from networks.classification import bert_qa_dragon as network

    if args.backbone == 'roberta':
        if args.baseline=='one':
            from approaches.classification import roberta_one as approach
            from networks.classification import roberta as network
        if args.baseline=='one_qa_dragon':
            from approaches.classification import roberta_one_qa_dragon as approach
            from networks.classification import roberta_qa_dragon as network

    


    if args.backbone == 'roberta_adapter':
        if args.baseline=='one':
            from approaches.classification import roberta_adapter_one as approach
            from networks.classification import roberta_adapter as network
        elif args.baseline=='one_qa_dragon':
            from approaches.classification import roberta_adapter_one_for_all_tasks_qa_dragon as approach
            from networks.classification import roberta_adapter_qa_dragon_all_tasks as network
        elif args.baseline=='ctr_kg_qa_dragon':
            if args.build_adapter_mask:
                from approaches.classification import roberta_qa_dragon_adapter_mask as approach
                from networks.classification import roberta_qa_dragon_adapter_mask as network
            else:
                from approaches.classification import roberta_qa_dragon_adapter_capsule_mask as approach
                from networks.classification import roberta_qa_dragon_adapter_capsule_mask as network
