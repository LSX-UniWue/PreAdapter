...
class Appr(object):
    ...
    def __init__(self,model,aux_model=None,logger=None,taskcla=None, args=None):
        ...
        self.thres_cosh=50
        self.thres_emb=6
        # self.lamb=0.75#10#2000#0.75
        self.lamb=args.mylamb#10#2000#0.75
    ...
    def hat_criterion_adapter(self,outputs,targets,masks):
        ...
        greatest_layer_size = 0
        greatest_layer_name = ''
        dict_with_large_layer = {}
        ...
        if self.mask_pre is not None:
            ...
            for m_key,m_value in masks.items():
                reg+=m_value.sum()
                count+=np.prod(m_value.size()).item()
                if np.prod(m_value.size()).item() >= greatest_layer_size:
                    greatest_layer_size = np.prod(m_value.size()).item()
                    greatest_layer_name = m_key
                    dict_with_large_layer[m_key] = np.prod(m_value.size()).item()
